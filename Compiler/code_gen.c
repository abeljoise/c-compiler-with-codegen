...
int isModuloOperator(Token* operator) {
    if(operator == NULL) {
        return 0;
    }

    if(operator->TOKEN_NAME == TK_MOD)
        return 1;

    // Lexeme fallback keeps codegen robust for malformed token metadata.
    return (operator->LEXEME != NULL && strcmp(operator->LEXEME, "%") == 0);
}

void loadArithmeticOperand(ASTNode* operandNode, SymbolTable* st, FILE* f, int recordArithmetic) {
    if(operandNode->LABEL == astArithmeticExpression) {
        codeGenerationHelper(operandNode, st, f, recordArithmetic);
    }
    else if(operandNode->LABEL == astNum) {
        fprintf(f,"mov ax, %s\n" ,operandNode->AST_NODE_TYPE.AST_NUM.VALUE->LEXEME);
    }
    else if(operandNode->LABEL == astId) {
        fprintf(f,"mov ax, [");
        printLeaf(operandNode,f);
        fprintf(f,"]\n");
    }
}
...
void codeGenerationHelper(ASTNode* node, SymbolTable* st, FILE* f, int recordArithmetic) {
    ...
    switch(label) {
        ...
        case astArithmeticExpression: {
            ...
            // Saving contents of bx and cx prior to using them further
            fprintf(f,"push bx\n");
            fprintf(f,"push cx\n");
			...
			if(recordArithmetic == 0) {
                ...
                loadArithmeticOperand(lhsNode, st, f, 0);

		        // Store first intermediate result on stack
		        fprintf(f,"push ax\n");

                loadArithmeticOperand(rhsNode, st, f, 0);
                // Preserve RHS from AX (needed as temporary across divide/multiply)
                fprintf(f,"mov cx,ax\n");
                // Restore LHS in AX
                fprintf(f,"pop ax\n");
		        ...
		        if(operator->TOKEN_NAME == TK_MUL) {
                    ...
                    fprintf(f,"push dx\n");
		            fprintf(f,"xor dx,dx\n"); // dx must be zero prior to a divide operation
		            fprintf(f,"div cx\n");
                    fprintf(f,"pop dx\n");
		        }
                else if(isModuloOperator(operator)) {
                    fprintf(f,"push dx\n");
                    fprintf(f,"xor dx,dx\n");
                    fprintf(f,"div cx\n");
                    fprintf(f,"mov ax,dx\n"); // remainder
                ...
                else if(operator->TOKEN_NAME == TK_DIV || isModuloOperator(operator)) {
                    int isModulo = isModuloOperator(operator);
					codeGenerationHelper(lhsNode,st,f,recordArithmetic); // Get result from lhs on stack
					int numberFields = recordArithmetic;
					int iterator;
					for(iterator = numberFields-1; iterator >= 0; iterator--) {
            ...
            // Restoring contents of bx and cx
            fprintf(f,"pop cx\n");
            fprintf(f,"pop bx\n");
            return;
            break;
