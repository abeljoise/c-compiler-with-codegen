...
Token* compatibleDataTypes(Token* t1, Token* t2,Token* operator) {
    if(operator->TOKEN_NAME == TK_MOD) {
        int lhsIsIntLike = (t1->TOKEN_NAME == TK_INT || t1->TOKEN_NAME == TK_NUM);
        int rhsIsIntLike = (t2->TOKEN_NAME == TK_INT || t2->TOKEN_NAME == TK_NUM);
        if(lhsIsIntLike && rhsIsIntLike)
            return t1;
        return NULL;
    }
    ...
    // Case when only one is TK_RECORDID, obviously a mismatch
        ...
        if((operator->TOKEN_NAME == TK_MUL || operator->TOKEN_NAME == TK_DIV) && t1->TOKEN_NAME == TK_RECORDID && (t2->TOKEN_NAME == TK_INT || t2->TOKEN_NAME == TK_REAL || t2->TOKEN_NAME == TK_NUM || t2->TOKEN_NAME == TK_RNUM))
            return t1;
        // Record with scalar in multiplication is valid when the record is the RHS.
        // Division with record on RHS is intentionally disallowed.
