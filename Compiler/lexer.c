...
void initializeBuffers(int f) {
    ...
    if(buffPair[0] == NULL || buffPair[1] == NULL) {
        printf("Error: failed to allocate input buffers for lexer\n");
        if(buffPair[0] != NULL) {
            free(buffPair[0]);
            buffPair[0] = NULL;
        }
    ...
    dfa_state = 0;
    lexerInitialized = (fp >= 0);
    if(lexerInitialized == 0) {
        printf("Error: invalid input file descriptor for lexer\n");
    }
}
...
int getInputStream() {
    ...
    if(res == -1) {
        printf("Error: failed to read input stream: %s\n", strerror(errno));
        return -1;
    }
...
Token* getToken() {
    ...
    while(1) {
        ...
        switch(dfa_state) {
            ...
            case 33: {
                char* lex = copyString(lexemeBegin,forward);
                populateToken(t,TK_MOD,lex,lineCount,0,NULL);
                accept();
                return t;
                break;
            }
...
void removeComments(char* testCaseFile, char* cleanFile) {
    ...
    if(tcf < 0) {
        printf("Error: cannot open input file %s for removing comments\n", testCaseFile);
        return;
    }
