...
#define TOTAL_GRAMMAR_TERMINALS 57
#define TOTAL_GRAMMAR_RULES 88 //TODO actual number of rules
    ...
    for(int i=1; i <= TOTAL_GRAMMAR_RULES; i++) {
        ...
        while(trav != NULL) {
            ...
            if(trav->IS_TERMINAL == 0) {
                ...
                // Case when FOLLOW(LHS) must be propagated to the current non-terminal
...
void createParseTable(FirstAndFollow* fafl, ParsingTable* pt) {
    ...
    for(int i=1; i <= TOTAL_GRAMMAR_RULES; i++) {
        ...
        // This naive approach is incorrect because unrelated FIRST entries can map to this rule.
        // for(int j=0; j < TOTAL_GRAMMAR_TERMINALS; j++) {
        //     if(fafl->FIRST[lhsNonTerminal],[j] == 1) {
        //         // Since grammar is LL(1), non-epsilon productions should not conflict on a filled entry.
        //         // Epsilon productions require separate FOLLOW-set handling.
...
Grammar* extractGrammar() {
    ...
    if(fd < 0) {
        printf("Error: unable to open grammar file %s\n", GRAMMAR_FILE);
        return NULL;
    }
...
ParseTree* parseInputSourceCode(char *testcaseFile, ParsingTable* pTable, FirstAndFollow* fafl) {
    ...
    if(isLexerInitialized() == 0) {
        printf("Error: lexer initialization failed for input source file %s\n", testcaseFile);
        syntaxErrorFlag = 1;
        close(f);
        return NULL;
    }
    ...
    if(pt == NULL) {
        printf("Error: failed to initialize parse tree\n");
        syntaxErrorFlag = 1;
        close(f);
        return NULL;
    }
    ...
    if(st == NULL) {
        printf("Error: failed to initialize parser stack\n");
        syntaxErrorFlag = 1;
        close(f);
        return NULL;
    }
